...
def  y(w,x,g,Fp,dr):
	return (w*p0*x+g*sum(Fp)*dr)[0]*0.000004
...
def sinpzt(t,f,a,o):
	return a*numpy.sin(2*numpy.pi*f*t+o)
...
def getbig(alpha,xn,yn,zn,nxn,nyn,nzn,invx,invy,invz):
	nxn+=alpha*(xn-invx)
	nyn+=alpha*(yn-invy)
	nzn+=alpha*(zn-invz)
	return nxn,nyn,nzn
...
def _plot():
	...
	#print('split to pzt')
	nx1n,ny1n,nz1n=getbig(alpha,x1n,y1n,z1n,nx1n,ny1n,nz1n,pztx,pzty,pztz)
	...
	nx2n,ny2n,nz2n=getbig(alpha,x2n,y2n,z2n,nx2n,ny2n,nz2n,reflectx,reflecty,reflectz)
...
def _plot4(lD):
	step1nD=exprstep1n.subs({'x':Glaserx,'y':Glasery,'z':Glaserz,'nx':Glasernx,'ny':Glaserny,'nz':Glasernz})
	step2nD=exprstep2n.subs({'x':Glaserx,'y':Glasery,'z':Glaserz,'nx':Glasernx,'ny':Glaserny,'nz':Glasernz})
	singall=[]
	coe=2*numpy.pi/Glamd

	for D in lD:
		step1nfinal=step1nD.subs({'dp':D})
		step2nfinal=step2nD.subs({'dp':D})
		R=math.cos(coe*step1nfinal)+math.cos(coe*step2nfinal)
		I=math.sin(coe*step1nfinal)+math.sin(coe*step2nfinal)
		singal=R**2+I**2
		singall.append(singal)
	return singall

def _plot3():
	wave=iptwave.get()
	freq=float(iptv1.get())
	Amp=float(iptv2.get())
	offset=float(iptvd.get())
	p0=2.42
	R=1
	M=1000
	dr=R/M
	...
	for k in range(K):
		if wave =='0':
			xl.append(sinpzt(tl[k],freq,Amp,offset))
		if wave =='1':
			xl.append(rectpzt(tl[k],freq,Amp,offset))
		if wave =='2':
			xl.append(ramppzt(tl[k],freq,Amp,offset))
		if wave =='3':
			xl.append(toothpzt(tl[k],freq,Amp,offset))
	vd=0
	before=xl[0]
	for k in range(K):
		x=xl[k]
	...
	plt.plot(tl,xl)
	plt.plot(tl,singall)
	plt.savefig('wave/'+wave+'  f='+str(freq)+' A='+str(Amp)+' o='+str(offset)+'.png')
	a.plot(tl,xl)
	a.plot(tl,singall)

	headers = ['time/s','input singal/V','output photocell/V']
	rows=[]
	for k in range(K):
		kr=[tl[k],xl[k],singall[k]]
		rows.append(kr)
	with open('csv/'+wave+'  f='+str(freq)+' A='+str(Amp)+' o='+str(offset)+'.csv','w',newline='')as ft:
		f_csv = csv.writer(ft)
...
labwave=tkinter.Label(root,text='波形0正弦1方波2斜波3锯齿')
labwave.place_forget()
iptwave=tkinter.Entry(root,highlightcolor='red',highlightthickness=1,textvariable=tkinter.StringVar(value='0'))
iptwave.place_forget()
...
labv1,iptv1,labiptv1 = getlabipt('频率','100',0.12,0.58)
labv2,iptv2,labiptv2 = getlabipt('振幅','5',0.12,0.58)
labvd,iptvd,labiptvd = getlabipt('偏移','0',0.12,0.58)
labv1.place_forget()
labv2.place_forget()
labvd.place_forget()
iptv1.place_forget()
...
def _lock():
	...
	labwave.place(relx=0.0,rely=0.74,anchor='nw',width=150,height=20)
	iptwave.place(relx=0.16,rely=0.74,anchor='nw',width=70,height=20)
